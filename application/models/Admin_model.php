...
class Admin_model extends CI_Model
{
    ...
    public function getBarangMasukId($id_barang)
    {
        $this->db->select('*');
    ...
    {
        $this->db->select('*,bm.gambar as gambar_bm,bm.berkas as berkas_bm');
        $this->db->join('user u', 'bm.user_id = u.id_user');
        $this->db->join('barang b', 'bm.barang_id = b.id_barang');
        $this->db->join('satuan s', 'b.satuan_id = s.id_satuan');
    ...
    public function getBarangKeluarId($id_barang)
    {
        $this->db->select('*,bk.gambar as gambar_bk, bk.berkas as berkas_bk');
        ...
        return $this->db->get('barang_keluar bk')->result();
    }

    public function getBarangKeluar($limit = null, $id_barang = null, $range = null)
    {
        $this->db->select('*,bk.gambar as gambar_bk');
        $this->db->join('user u', 'bk.user_id = u.id_user');
        $this->db->join('barang b', 'bk.barang_id = b.id_barang');
        $this->db->join('satuan s', 'b.satuan_id = s.id_satuan');
        $this->db->join('status st', 'bk.status_id = st.id_status');
